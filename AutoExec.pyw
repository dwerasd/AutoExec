...
def db_init():
    ...
    """)
    # window_rules에 monitor_name 컬럼 추가 (서브 모니터 모드의 타깃 모니터 이름)
    try:
        cur.execute("ALTER TABLE window_rules ADD COLUMN monitor_name TEXT NOT NULL DEFAULT ''")
    except Exception:
        pass
    # ── 과제 (루틴) 테이블 ──
    cur.execute("""
        ...
        if profile_count == 0:
            ...
            for ot in old_targets:
                ...
                cur.execute(
                    ...
                    "target_x, target_y, target_w, target_h, maximize, monitor_name, sort_order) "
                    "VALUES (?, '', '', ?, ?, ?, ?, ?, ?, '', 0)",
...
def db_touch_last_routine_log(routine_id, log_date):
    """특정 과제의 특정 날짜 마지막 완료 기록의 done_time을 현재 시각으로 갱신.
    갱신 성공 시 새 datetime 문자열 반환, 기록 없으면 None."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute(
        ...
        if not row:
            return None
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        conn.execute("UPDATE routine_logs SET done_time=? WHERE id=?", (now_str, row["id"]))
        conn.commit()
        return now_str
    finally:
        conn.close()
...
def db_upsert_rule(rule_id, profile_id, window_class="", title_pattern="",
                   move_mode="custom", target_x=0, target_y=0, target_w=0, target_h=0,
                   maximize=0, monitor_name=""):
    """규칙 추가 또는 수정. 반환: id"""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        if rule_id:
            cur.execute(
                ...
                "target_x=?, target_y=?, target_w=?, target_h=?, maximize=?, monitor_name=? WHERE id=?",
                (profile_id, window_class, title_pattern, move_mode,
                 target_x, target_y, target_w, target_h, int(maximize), monitor_name, rule_id),
            )
        else:
            cur.execute(
                "INSERT INTO window_rules (profile_id, window_class, title_pattern, move_mode, "
                ...
                (profile_id, window_class, title_pattern, move_mode,
                 target_x, target_y, target_w, target_h, int(maximize), monitor_name, profile_id),
            )
        ...
        for idx, r in enumerate(rules_list):
            cur.execute(
                ...
                (profile_id, r.get("window_class", ""), r.get("title_pattern", ""),
                 ...
                 r.get("target_w", 0), r.get("target_h", 0), int(r.get("maximize", 0)),
                 r.get("monitor_name", ""), idx),
            )
        conn.commit()
    finally:
        conn.close()
...
def _parse_edid_id(device_id):
    """DISPLAY_DEVICEW.DeviceID에서 EDID 하드웨어 ID 추출.
    예: 'MONITOR\\DELA0A4\\{4d36e96e-...}\\0001' → 'DELA0A4'
    """
    if not device_id:
        return ""
...
def _format_monitor_label(monitor):
    r"""모니터 dict를 '이름 [EDID]' 형식 라벨로 포맷.
    이름만 있으면 이름, EDID만 있으면 EDID, 둘 다 없으면 device(\\.\DISPLAY1).
    """
    if not monitor:
        return ""
    name = (monitor.get("name") or "").strip()
    edid = (monitor.get("edid_id") or "").strip()
    if name and edid:
        return f"{name} [{edid}]"
    if name:
        return name
    if edid:
        return f"[{edid}]"
    return monitor.get("device", "") or ""


def _enumerate_monitors():
    """모니터 정보 반환:
    [{x, y, w, h, is_primary, device, name, device_id, edid_id}, ...]
    device: '\\.\\DISPLAY1' 등 어댑터 식별자
    name: EDID 기반 친화적 모니터 이름 (예: 'DELL U2720Q')
    device_id: 전체 디바이스 인스턴스 경로 (예: 'MONITOR\\DELA0A4\\{...}\\0001')
    edid_id: EDID 하드웨어 코드 (예: 'DELA0A4')
    """
    monitors = []
    ...
    def callback(hMonitor, hdcMonitor, lprcMonitor, dwData):
        ...
        # 모니터의 EDID 친화 이름 + 디바이스 인스턴스 ID 조회 (ACTIVE 슬롯만)
        friendly = ""
        device_id = ""
        try:
            idx = 0
            while True:
                ...
                if not user32.EnumDisplayDevicesW(info.szDevice, idx, ctypes.byref(dev), 0):
                    break
                if dev.StateFlags & 0x1:  # DISPLAY_DEVICE_ACTIVE
        ...
        monitors.append({
            ...
            "name": friendly,
            "device_id": device_id,
            "edid_id": _parse_edid_id(device_id),
        })
        return True
...
#  WOL (Wake-on-LAN)
# ═══════════════════════════════════════════════════════════
def _find_local_ip_for_subnet(target_ip):
    """target_ip와 같은 /24 서브넷의 로컬 NIC IP 탐색.
    Hyper-V/WSL/VMware 가상 NIC 우회 위해 송신 socket bind에 사용.
    """
    try:
        ...
        if len(parts) != 4:
            return None
        prefix = ".".join(parts[:3]) + "."
        # 모든 로컬 IPv4 주소 수집 → 동일 /24 서브넷 IP 우선
...
def send_wol(mac_str, target_ip=None):
    """매직 패킷 전송.
    limited broadcast + 서브넷 directed broadcast + 유니캐스트, 포트 7/9 송신.
    target_ip와 같은 서브넷의 로컬 IP가 있으면 socket을 거기에 bind하여
    가상 NIC(WSL/Hyper-V Firewall/VMware) 우회 후 실제 LAN으로 강제 송출.
    """
    mac_str = mac_str.replace("-", "").replace(":", "")
    if len(mac_str) != 12:
        return False, "MAC 형식 오류"
    try:
        mac_bytes = bytes.fromhex(mac_str)
    except ValueError:
        return False, "MAC 16진수 변환 실패"
    ...
    local_ip = _find_local_ip_for_subnet(target_ip) if target_ip else None
    ...
    for tgt in targets:
        for port in (9, 7):
            try:
                ...
                errs.append(f"{tgt}:{port} - {e}")
    if sent_any:
        bind_info = f" via {local_ip}" if local_ip else " (NIC 자동 선택)"
        return True, (bind_info if not errs else f"{bind_info}, 일부 실패: {'; '.join(errs)}")
    return False, f"모든 송신 실패: {'; '.join(errs)}"


def ping_host(ip, timeout=2):
    """ping 으로 호스트 응답 확인.
    Windows ping은 'Destination host unreachable' ICMP error 시에도 returncode 0 반환.
    실제 응답 여부는 출력에 TTL이 포함됐는지로 판단.
    """
    try:
...
def wol_boot_thread(mac, ip, pc_name, log_callback):
    ...
    for attempt in range(1, 3):
        ok, info = send_wol(mac, ip)
        if ok:
            log_callback(f"[WOL] {pc_name}({ip}) 매직 패킷 전송 ({attempt}차){info or ''}")
        else:
            log_callback(f"[WOL] {pc_name}({ip}) 매직 패킷 전송 실패 ({attempt}차) - {info}")
...
class RuleEditDialog(tk.Toplevel):
    ...
    def __init__(self, parent, rule=None, exe_name="", monitor_names=None):
        super().__init__(parent)
        self.result = None
        self.rule = rule
        self._exe_name = exe_name
        self._monitor_names = list(monitor_names or [])
        ...
        # 타깃 모니터 선택 (서브 모니터 모드 전용, 그 외에는 비활성)
        self.monitor_frame = ttk.Frame(frame)
        self.monitor_frame.grid(row=7, column=0, columnspan=4, sticky=tk.W, pady=3)
        ttk.Label(self.monitor_frame, text="타깃 모니터:").pack(side=tk.LEFT)
        self.var_monitor = tk.StringVar(value="")
        self.cmb_monitor = ttk.Combobox(self.monitor_frame, textvariable=self.var_monitor,
                                        values=self._monitor_names, width=40)
        self.cmb_monitor.pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(frame, text="(서브 모니터 모드 전용 - 비워두면 첫 번째 비주 모니터)",
                  foreground="gray").grid(row=8, column=0, columnspan=4, sticky=tk.W)

        self.var_maximize = tk.BooleanVar(value=False)
        ttk.Checkbutton(frame, text="최대화", variable=self.var_maximize).grid(
            row=9, column=0, columnspan=2, sticky=tk.W, pady=3)
        ...
        if rule:
            ...
            self.var_monitor.set(rule.get("monitor_name", "") or "")
        ...
        btn_frame = ttk.Frame(frame)
        btn_frame.grid(row=10, column=0, columnspan=4, pady=(10, 0))
        ttk.Button(btn_frame, text="확인", command=self._on_ok).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="취소", command=self.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def _on_mode_changed(self, event=None):
        ...
        # 모니터 콤보박스는 항상 보이며, sub_monitor 모드에서만 활성화
        self.cmb_monitor.config(state="normal" if mode == "sub_monitor" else "disabled")
    ...
    def _on_ok(self):
        ...
        self.result = {
            ...
            "monitor_name": self.var_monitor.get().strip() if mode == "sub_monitor" else "",
        }
        self.destroy()
...
class ProfileEditDialog(tk.Toplevel):
    ...
    def _refresh_rules_tree(self):
        ...
        for idx, r in enumerate(self.rules_data):
            ...
            if mode in ("custom", "save_position") and (x or y or w or h):
                size = f" {w}x{h}" if w and h else ""
                mode_str += f" ({x},{y}{size})"
            elif mode == "sub_monitor":
                mon = r.get("monitor_name", "") or "(첫 비주)"
                mode_str += f" [{mon}]"
    ...
    def _current_monitor_names(self):
        """현재 연결된 모니터의 '이름 [EDID]' 라벨 목록 반환 (RuleEditDialog 콤보박스용).
        주 모니터 라벨에는 '(주)' 접미사 부착.
        """
        try:
    ...
    def _add_rule(self):
        exe = self.ent_exe.get().strip()
        dlg = RuleEditDialog(self, exe_name=exe, monitor_names=self._current_monitor_names())
    ...
    def _edit_rule(self):
        ...
        dlg = RuleEditDialog(self, rule=self.rules_data[idx], exe_name=exe,
                             monitor_names=self._current_monitor_names())
        self.wait_window(dlg)
        if dlg.result:
...
class AutoExecApp:
    ...
    def _on_routine_right_click(self, event):
        ...
        menu.add_command(label="현재 시간으로 갱신", command=self._touch_routine)
    ...
    def _touch_routine(self):
        """선택한 일정의 마지막 완료 시간을 현재 시각으로 갱신
        (새벽에 완료 후 같은 날 다시 수행한 경우 시간만 업데이트)"""
        rt = self._get_selected_routine()
        if not rt:
            return
        active_date = self._get_selected_routine_date()
        if not active_date:
            return
        new_time = db_touch_last_routine_log(rt["id"], active_date)
        if not new_time:
            messagebox.showinfo("알림", f"'{rt['name']}' 갱신할 기록이 없습니다.",
                                parent=self.root)
            return
        self._refresh_routine_list()
        self.log(f"일정 시간 갱신: {rt['name']} ({active_date} → {new_time[11:]})")
    ...
    def _apply_profile_rules(self, profile, rules, monitors_info=None):
        """프로파일의 규칙에 따라 창 이동. 반환: 이동된 창 수.
        monitors_info: 사전 조회된 모니터 리스트 (None이면 sub_monitor 규칙이 있을 때만 조회)
        """
        ...
        for w in windows:
            ...
            try:
                ...
                if move_mode == "sub_monitor":
                    target_mon = self._resolve_target_monitor(monitors or [], rule)
                    if not target_mon:
                        target_name = rule.get("monitor_name", "") or "(비주 모니터)"
                        self.log(f"[프로파일] 모니터 '{target_name}' 미감지 → 이동 생략")
                        continue
                    tx, ty = target_mon["x"], target_mon["y"]
                    tw, th = target_mon["w"], target_mon["h"]
    ...
    def _check_monitor_change(self):
        ...
        # 모니터 개수가 변하고 2개 이상이면 트리거 (1→2, 1→3, 2→3, 3→2 등)
        if count != prev and count >= 2:
            self.log(f"[모니터] 모니터 수 변화 감지 ({prev}→{count}) → 프로파일 기반 창 이동")
    ...
    def _get_monitors_info(self):
        """모니터 정보 반환 (모듈 함수 _enumerate_monitors 래핑)"""
        return _enumerate_monitors()

    def _resolve_target_monitor(self, monitors, rule):
        """규칙의 monitor_name을 모니터 리스트에서 찾아 반환.
        monitor_name은 '이름 [EDID]' 라벨 또는 레거시(이름만)을 모두 허용.
        매칭 우선순위:
          1. 라벨 전체 일치 (이름 + EDID)
          2. 라벨에 포함된 EDID 일치 (이름이 바뀌어도 OK)
          3. 이름만 일치 (구버전 레거시)
          4. 비주 모니터 중 첫 번째 (폴백)
        """
        target = (rule.get("monitor_name", "") or "").strip()
        # UI에서 부착되는 '(주)' 접미사 제거
        if target.endswith(" (주)"):
            target = target[:-len(" (주)")].strip()
        if target:
            # EDID 부분 분리: '...[EDID]' → 'EDID'
            target_edid = ""
            if "[" in target and target.endswith("]"):
                target_edid = target[target.rindex("[") + 1:-1].strip()
            target_name_only = target.split("[", 1)[0].strip() if "[" in target else target

            # 1) 전체 라벨 일치
            for m in monitors:
                if _format_monitor_label(m) == target:
                    return m
            # 2) EDID 일치
            if target_edid:
                for m in monitors:
                    if m.get("edid_id", "") == target_edid:
                        return m
            # 3) 이름만 일치 (레거시)
            if target_name_only:
                for m in monitors:
                    if m.get("name", "") == target_name_only:
                        return m
        # 4) 폴백: 첫 번째 비주 모니터
        for m in monitors:
            if not m["is_primary"]:
    ...
    def _on_monitor_change(self):
        """모니터 수 변화 감지 시 trigger_mode이 monitor_change/both인 프로파일 실행"""
        try:
            time.sleep(2)  # 모니터 인식 안정화 대기

            monitors = self._get_monitors_info()
            mon_summary = ", ".join(
                f"{m['name'] or m['device']}{'*' if m['is_primary'] else ''}"
                for m in monitors
            )
            self.log(f"[모니터] 감지된 모니터: {mon_summary}")
            ...
            for profile in profiles:
                ...
                if not rules:
                    continue
                moved = self._apply_profile_rules(profile, rules, monitors_info=monitors)
                total_moved += moved
