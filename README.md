...
- **마우스 커서 설치**: `cursors\skinpack\` 의 커서를 `%SystemRoot%\Cursors\skinpack` 에 복사하고 구성표 등록·현재 커서 적용·즉시 반영·검증까지 한 번에 수행 (관리자 권한 필요). KB5120998 한국어 리소스가 커서 값 이름 6개를 번역해 조회하는 버그도 같은 값을 한글 이름으로 함께 써서 보정
